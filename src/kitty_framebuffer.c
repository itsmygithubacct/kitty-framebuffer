    ...
    size_t rect_count)
{
    ...
    /*
     * Claim the process-wide encoder before consuming a pending frame or
     * editing the displayed image.  The per-session output lock then covers
     * the complete escape-stream burst, preserving the later main-branch
     * guarantee that a synchronous patch cannot splice into a full frame.
     *
